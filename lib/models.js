...
Image.prototype = Object.create(Configurable.prototype);

Image.prototype.setNames = function (name) {
  this.name = name;
  // this.name = namer.unwikify(name.replace(/\.md$/, ""));
  this.wikiname = this.name;
...
Image.prototype.remove = function () {
  ...
  return new Promiserr(function (resolve, reject) {
    ...
    gitmech.rm(this.filename, "Image removed (" + this.wikiname + ")", this.author, function (err) {
      resolve();
    });
  }.bind(this));
};

Image.prototype.renameTo = function (newName) {
  ...
  return new Promiserr(function (resolve, reject) {
    ...
    gitmech.mv(this.filename,
               newFilename,
               "Image renamed (" + this.filename + " => " + newFilename + ")",
               this.author,
               function (err) {
                 if (err) {
                   reject();
...
Image.prototype.savePipeable = function (pipeable) {
  var self = this;
  var message = message || "";
  ...
  return new Promiserr(function (resolve, reject) {
    ...
    var fname = self.name;
    // var defMessage = (this.exists() ? "Content updated" : "Image created") + " (" + this.wikiname + ")";


    self.gitDestPath = gitmech.absPath(fname);
    self.gitTempPath = gitmech.absPath('__temp__'+fname);
    log_dbg('uploading to: ',self.gitTempPath);

    var stream = fs.createWriteStream(self.gitTempPath);
    pipeable.pipe(stream);
    stream.on('close',function(){
      log_dbg("ok, finished writing image. Need to add to git");
      resolve(self.gitTempPath);
    });

    stream.on('error',function(err){
      log_dbg("Error on upload:",err);
      reject(err);
    });

    // if (this.getConfig().Images.title.fromContent) {
    //   content = "# " + this.title + "\n" + content;
    // }
...
Image.prototype.commit = function() {
  var self = this;
  return new Promiserr(function (resolve, reject) {
    if(self.gitDestPath && self.gitTempPath) {
      fs.rename(self.gitTempPath,self.gitDestPath,function(err){
        if(!err) {
          var message = "Image uploaded " + self.gitDestPath;
          gitmech.add(self.gitDestPath,message,self.author,function(err){
            if(err) {
              console.error("Error adding image to git repo:",err)
              reject(err);
            } else {
              log_dbg("image",self.gitDestPath,"committed to git repo.");
              resolve(path.basename(self.gitDestPath));
            }
          }); 
        } else {
          console.error("Error moving temp file:",self.gitTempPath,self.gitDestPath);
          reject(err);
        }
      });
    } else {
      reject("No gitDestPath set.");
    }
  });
}

Image.prototype.cleanup = function() {
  var self = this;
  return new Promiserr(function (resolve, reject) {
    if(self.gitTempPath)
      fs.unlink(self.gitTempPath,function(err){
        resolve();
      });
    else
      resolve();
...
Image.prototype.save = function (message) {
...
Image.prototype.urlFor = function (action) {

  return Image.urlFor(this.wikiname, action, this.getProxyPath());
};

Image.urlFor = function (name, action, proxyPath) {
  ...
  switch (true) {
    ...
    case action == "edit":
      url = "/Images/" + wname + "/edit";
      break;

    case action == "edit error":
      url = "/Images/" + wname + "/edit?e=1";
      break;
    ...
    case action == "new":
      url = "/Images/new/" + wname;
      break;

    case action == "new error":
      url = "/Images/new/" + wname + "?e=1";
      break;
...
Image.prototype.urlForShow = function (action) {
  return this.urlFor("show");
};

Image.prototype.urlForEdit = function (action) {
  return this.urlFor("edit");
};

Image.prototype.urlForEditWithError = function (action) {
  return this.urlFor("edit error");
};

Image.prototype.urlForNewWithError = function (action) {
  return this.urlFor("new error");
};

Image.prototype.urlForEditPut = function (action) {
  return this.urlFor("edit put");
};

Image.prototype.urlForRevert = function (action) {
  return this.urlFor("revert");
};

Image.prototype.urlForHistory = function (action) {
  return this.urlFor("history");
};

Image.prototype.urlForCompare = function (action) {
  return this.urlFor("compare");
};

Image.prototype.isIndex = function () {
  return this.getConfig().Images.index == this.name;
};

Image.prototype.isFooter = function () {
  return this.name == "_footer";
};

Image.prototype.isSidebar = function () {
  return this.name == "_sidebar";
};

Image.prototype.lock = function (user) {
...
Image.prototype.unlock = function (user) {
  this.lockedBy = null;
  locker.unlock(this.name);
};

Image.prototype.fetch = function (extended) {
...
Image.prototype.fetchContent = function () {
  ...
  return new Promiserr(function (resolve, reject) {
    ...
    gitmech.show(this.filename, this.revision, function (err, content) {
      ...
      if (err) {
        ...
        if (content.length === 0 || this.getConfig().Images.title.fromFilename) {
          this.title = this.name;
          this.content = content;
        }
        else {
...
Image.prototype.fetchMetadata = function () {
...
Image.prototype.fetchHashes = function (howmany) {
...
Image.prototype.fetchLastCommitMessage = function () {
...
Image.prototype.fetchHistory = function () {
...
Image.prototype.fetchRevisionsDiff = function (revisions) {
...
Image.prototype.revert = function () {
...
Images.prototype = Object.create(Configurable.prototype);

Images.prototype.fetch = function (Imagen) {
  ...
  return new Promiserr(function (resolve, reject) {
    ...
    gitmech.ls("*.md", function (err, list) {
      ...
      var itemsPerImage = this.getConfig().Images.itemsPerImage;

      this.total = list.length;
      this.totalImages = Math.ceil(this.total / itemsPerImage);

      if (Imagen <= 0) {
        Imagen = 1;
      }
      if (Imagen > this.totalImages) {
        Imagen = this.totalImages;
      }

      this.currentImage = Imagen;

      // Read the stats from the fs to be able to sort the whole
      // list before slicing the Image out
      var listWithData = list.map(function (Image) {

        var stats;

        try {
          stats = fs.statSync(gitmech.absPath(Image));
        }
        catch (e) {
          stats = null;
        }
      ...
      var offset = (Imagen - 1) * itemsPerImage;
      var slice = listWithData.slice(offset, offset + itemsPerImage);

      slice.forEach(function (data) {
        var Image = path.basename(data.name).replace(/\.md$/,"");

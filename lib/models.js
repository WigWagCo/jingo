...
TOC.prototype._makeNewTOC = function(toc_path,cb) {
  var self = this;
  log_dbg("_makeNewTOC");
  return new Promiserr(function (resolve, reject) {
    ...
    if(toc_pieces.length > 0 && toc_pieces[0].length > 0) {
      ...
      template_toc.name = toc_pieces[0];
      var walk = template_toc.leafs;
      for(var n=1;n<toc_pieces.length;n++) {
        walk[toc_pieces[n]] = {
          name: toc_pieces[n],
          pageURL: '/wiki/'+namer.wikify(toc_pieces[n]), // FIXME
          leafs: {}
        }
        walk = walk.leafs;
      }
      ...
      self.app.jingoMetadataDB.find({name:self.name}, function(err,docs) {
        if(err) {
          ...
          if(docs.length > 0) {
            ...
            self._makeNewTOC.then(function(doc){
              resolve(doc);
            });
          }
        }         
                ...
                for(var n=1;n<toc_pieces.length;n++) {
                  if(!walk[toc_pieces[n]]) {
                    ...
                    if(n == toc_pieces.length-1) { // if this is a leaf entry (no children)
                      ...
                      delete walk[toc_pieces[n]];  // delete the entry.
                      break;
                    }
                  }
                  // move down leaf
                  walk = walk[toc_pieces[n]].leafs;
                }
              }
